...
def create_canonical_data(fda_filename, field):
    ...
    print(fda.describe())
    fda[field] = fda[field].str.lower()
    fda_deduped = pandas_dedupe.dedupe_dataframe(fda, [field])
    fda_canonical = fda_deduped.drop_duplicates(subset=[field])
    fda_canonical.to_csv(f'data/csvs/canonical_{field}.csv', columns=[field], 
                         index=False)
